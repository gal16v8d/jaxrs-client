...
import com.gsdd.jaxrs.client.HTTPClient;
...
public class JaxClient implements HTTPClient {
  ...
  private static final String ERROR_EXECUTING =
      "Error when execute %s method for target -> '%s' . %s";
  ...
  protected Response invokePost(RestCallParams params) {
    ...
    try {
      return getInvocationBuilder(params)
          .post(Entity.entity(params.getPayload(), params.getAcceptedMimeType()));
    } catch (Exception e) {
  ...
  protected Response invokePut(RestCallParams params) {
    try {
      log.debug("About to execute PUT method for target {} with payload -> {}", params.getTarget(),
          params.getPayload());
      return getInvocationBuilder(params)
          .put(Entity.entity(params.getPayload(), params.getAcceptedMimeType()));
    } catch (Exception e) {
  ...
  protected Response invokePatch(RestCallParams params) {
    try {
      log.debug("About to execute PATCH method for target {} with payload -> {}",
          params.getTarget(), params.getPayload());
      return getInvocationBuilder(params)
          .build("PATCH", Entity.entity(params.getPayload(), params.getAcceptedMimeType()))
          .invoke();
    } catch (Exception e) {
  ...
  protected <E> E callEndpointAndGetEntity(Response response, RestCallParams params,
      GenericType<E> entity) {
    try {
