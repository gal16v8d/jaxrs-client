...
class DefaultResponseHandlerTest {
  ...
  void mustlaunchExcWhenResponseIsBadTest() {
    ...
    Mockito.when(responseStatus.getStatusCode())
        .thenReturn(Status.INTERNAL_SERVER_ERROR.getStatusCode());
    HTTPClientException e = Assertions.assertThrows(HTTPClientException.class,
        () -> defaultResponseHandler.processResponse(logger, response));
    Assertions.assertEquals("Service responses with a status: 500 reason: " + MOCKED_ERROR,
        e.getMessage());
  }
  ...
  void mustNotlaunchExcWhenResponseIsGoodTest() {
    ...
    Mockito.verify(logger).debug(Mockito.eq("Service responses with status -> {}"),
        Mockito.anyInt());
